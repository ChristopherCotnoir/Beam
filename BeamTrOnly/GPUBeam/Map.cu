...
void dumpBeamByThread(Simulator *sim, double *&d_x, double *&h_x, int Npart, int Ncol, int iTurn, std::string ic, std::_Ios_Openmode mode){
  ...
  if (bParams->Nfreq > 0 && (iTurn % bParams->Nfreq == 0 || io->pending_log)) {
    ...
    QuadDebug(cudaMemcpy(h_x, d_x, sizeof(double) * NCOL * Npart, cudaMemcpyDeviceToHost));
    io->log_thread = std::thread(&InputProcessing::dumpParticles, io, h_x, Npart, Ncol, bParams->Nfreq, iTurn, "dump.ebeam",mode, sim->rank);
...
void dumpBeamByThread(Simulator *sim, double *&d_x, double *&h_x, int Npart, int Ncol, int iTurn, std::string ic, std::_Ios_Openmode mode){
  ...
  if (bParams->Nfreq > 0 && (iTurn % bParams->Nfreq == 0)) {
    ...
    std::cout << ss.str() << std::endl;
    QuadDebug(cudaMemcpy(h_x, d_x, sizeof(double) * NCOL * Npart, cudaMemcpyDeviceToHost));
    io->dumpParticles(h_x, Npart, Ncol, bParams->Nfreq, iTurn, "dump.ebeam", mode, sim->rank);
  }
}
...
initDeviceMemory(int *&ditM, int *&hitM,
  ...
  QuadDebug(cudaMalloc((void **)&dx, sizeof(double) * Npart * Ncol));


  QuadDebug(cudaMemcpy(dNrow, hNrow, sizeof(int) * Ncol, cudaMemcpyHostToDevice));
  QuadDebug(cudaMemcpy(dx, x, sizeof(double) * Ncol * Npart, cudaMemcpyHostToDevice));
...
__global__
void
applyMSHFL1(int const* __restrict__ itM, double *x, double *op, int Npart, int maxLen, int iNrow, int attribId){
  int ie = blockIdx.x * BLOCKDIMX + threadIdx.x;
  ...
  xx = applyMapByAtrib(itM, x0, x1, x2, x3, x4, x5, Npart, iNrow, maxLen, attribId);
  if(ie < Npart){
    op[attribId * Npart + ie] = xx;
  }
}
...
computeRMS(double *dx, int Npart, 
	   ...
	   double &pxbar, double &pybar, double &pzbar,
	   double &sig_px, double &sig_py, double &sig_pz){

  thrust::device_ptr<double> ptr;
  ptr = thrust::device_pointer_cast(dx);
  xbar = thrust::reduce(ptr, ptr + Npart, (double) 0, thrust::plus<double>())/(double)Npart;
  ybar = thrust::reduce(ptr + 2 * Npart, ptr + 3 * Npart, (double) 0, thrust::plus<double>())/(double)Npart;
  zbar = thrust::reduce(ptr + 4 * Npart, ptr + 5 * Npart, (double) 0, thrust::plus<double>())/(double)Npart;


  pxbar = thrust::reduce(ptr + Npart, ptr + 2 * Npart, (double) 0, thrust::plus<double>())/(double)Npart;
  pybar = thrust::reduce(ptr + 3 * Npart, ptr + 4 * Npart, (double) 0, thrust::plus<double>())/(double)Npart;
  pzbar = thrust::reduce(ptr + 5 * Npart, ptr + 6 * Npart, (double) 0, thrust::plus<double>())/(double)Npart;
  ...
  sig_x = sqrt(thrust::reduce(ptr, ptr + Npart, (double) 0, thrust::plus<double>())/(double)Npart);
  
  ptr = thrust::device_pointer_cast(dy0);
  sig_y = sqrt(thrust::reduce(ptr, ptr + Npart, (double) 0, thrust::plus<double>())/(double)Npart);

  ptr = thrust::device_pointer_cast(dz0);
  sig_z = sqrt(thrust::reduce(ptr, ptr + Npart, (double) 0, thrust::plus<double>())/(double)Npart);

  ptr = thrust::device_pointer_cast(dx1);
  sig_px = sqrt(thrust::reduce(ptr, ptr + Npart, (double) 0, thrust::plus<double>())/(double)Npart);

  ptr = thrust::device_pointer_cast(dy1);
  sig_py = sqrt(thrust::reduce(ptr, ptr + Npart, (double) 0, thrust::plus<double>())/(double)Npart);

  ptr = thrust::device_pointer_cast(dz1);
  sig_pz = sqrt(thrust::reduce(ptr, ptr + Npart, (double) 0, thrust::plus<double>())/(double)Npart);
  ...
  standardized_moments<<<numBlocks, numThreads>>>(dx, &dx[2 * Npart], &dx[4 * Npart],
						  ...
						  Npart);
  ptr = thrust::device_pointer_cast(dx0);
  mom_x[0] = thrust::reduce(ptr, ptr + Npart, (double) 0, thrust::plus<double>())/(double)Npart;
  
  ptr = thrust::device_pointer_cast(dy0);
  mom_y[0] = thrust::reduce(ptr, ptr + Npart, (double) 0, thrust::plus<double>())/(double)Npart;
  
  ptr = thrust::device_pointer_cast(dz0);
  mom_z[0] = thrust::reduce(ptr, ptr + Npart, (double) 0, thrust::plus<double>())/(double)Npart;
  
  ptr = thrust::device_pointer_cast(dx1);
  mom_x[1] = thrust::reduce(ptr, ptr + Npart, (double) 0, thrust::plus<double>())/(double)Npart - 3.0;
  
  ptr = thrust::device_pointer_cast(dy1);
  mom_y[1] = thrust::reduce(ptr, ptr + Npart, (double) 0, thrust::plus<double>())/(double)Npart - 3.0;
  
  ptr = thrust::device_pointer_cast(dz1);
  mom_z[1] = thrust::reduce(ptr, ptr + Npart, (double) 0, thrust::plus<double>())/(double)Npart - 3.0;
}
