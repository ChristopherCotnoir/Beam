...
class InputProcessing:public IO{
 public:
  ...
  void dumpParticles(double *, int , int , int , int , std::string , std::_Ios_Openmode mode = std::ios::app, int pid = 0);
  void dumpLum(int iTurn, double Lc, double Lsl, int N,
	       ...
	       double pxbar_p, double pybar_p, double pzbar_p, double sig_px_p, double sig_py_p, double sig_pz_p, std::string sFile, std::_Ios_Openmode mode = std::ios::app);
   ...
   * \brief Debug macro
   */
#define Debug(e) IO::IODebug((e), __FILE__, __LINE__)

#define Abort(e) IO::IOAbort((e), __FILE__, __LINE__)
